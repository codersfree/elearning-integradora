...
public class CourseController {
    ...
    public String index(
        ...
        model.addAttribute("courses", courseService.searchAndFilterCourses(
            searchTerm, categoryIds, levelIds, priceIds, pageable
        ));
    ...
    // --- ENDPOINTS DEL PLAYER / PREVISUALIZACIÓN ---
    ...
    public String learn(@PathVariable String slug, Principal principal) {
        ...
        boolean isEnrolled = user.getCourses().stream().anyMatch(c -> c.getId().equals(course.getId()));
        if (!isEnrolled) {
    ...
    public String learnLesson(
        ...
        List<Lesson> allLessons = course.getModules().stream()
                ...
                .flatMap(m -> m.getLessons().stream().sorted(Comparator.comparing(Lesson::getPosition)))
    ...
    public String toggleLesson(
        ...
        if (lesson != null) {
            ...
            userService.save(user); // Guardamos la actualización de lecciones completadas
        }
